...
public class AppointmentsApiTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task POST_TherapistConflict_Returns409()
    {
        // Two different patients try to book the same therapist at the same slot
        var (_, t, _) = await SeedAsync("tc1");
        var locationId = await SeedData.CreateLocationAsync(_fixture.Client);
        var p2 = await SeedData.CreatePatientAsync(_fixture.Client, "tc2");
        var r2 = await SeedData.CreateRoomAsync(_fixture.Client, locationId, "tc2");
        var (p1, _, r1) = await SeedAsync("tc0");

        (await BookAsync(p1, t, r1, Slot9am)).StatusCode.Should().Be(HttpStatusCode.Created);

        var conflict = await BookAsync(p2, t, r2, Slot9am);
        conflict.StatusCode.Should().Be(HttpStatusCode.Conflict);
    }

    [Fact]
    public async Task POST_RoomConflict_Returns409()
    {
        var locationId = await SeedData.CreateLocationAsync(_fixture.Client);
        var r = await SeedData.CreateRoomAsync(_fixture.Client, locationId, "rc");
        var p1 = await SeedData.CreatePatientAsync(_fixture.Client, "rc1");
        var t1 = await SeedData.CreateTherapistAsync(_fixture.Client, "rc1");
        var p2 = await SeedData.CreatePatientAsync(_fixture.Client, "rc2");
        var t2 = await SeedData.CreateTherapistAsync(_fixture.Client, "rc2");

        (await BookAsync(p1, t1, r, Slot9am)).StatusCode.Should().Be(HttpStatusCode.Created);

        var conflict = await BookAsync(p2, t2, r, Slot9am);
        conflict.StatusCode.Should().Be(HttpStatusCode.Conflict);
    }

    [Fact]
    public async Task POST_PatientDoubleBooked_Returns409()
    {
        var locationId = await SeedData.CreateLocationAsync(_fixture.Client);
        var p = await SeedData.CreatePatientAsync(_fixture.Client, "pb");
        var t1 = await SeedData.CreateTherapistAsync(_fixture.Client, "pb1");
        var r1 = await SeedData.CreateRoomAsync(_fixture.Client, locationId, "pb1");
        var t2 = await SeedData.CreateTherapistAsync(_fixture.Client, "pb2");
        var r2 = await SeedData.CreateRoomAsync(_fixture.Client, locationId, "pb2");

        (await BookAsync(p, t1, r1, Slot9am)).StatusCode.Should().Be(HttpStatusCode.Created);

        var conflict = await BookAsync(p, t2, r2, Slot9am);
        conflict.StatusCode.Should().Be(HttpStatusCode.Conflict);
    }
    ...
    [Fact]
    public async Task GET_AppointmentById_Returns200()
    {
        var (p, t, r) = await SeedAsync("gid");
        var createResponse = await BookAsync(p, t, r, Slot9am);
        var doc = await JsonDocument.ParseAsync(await createResponse.Content.ReadAsStreamAsync());
        var id = doc.RootElement.GetProperty("id").GetInt32();

        var response = await _fixture.Client.GetAsync($"/api/appointments/{id}");

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var getDoc = await JsonDocument.ParseAsync(await response.Content.ReadAsStreamAsync());
        getDoc.RootElement.GetProperty("id").GetInt32().Should().Be(id);
    }

    [Fact]
    public async Task GET_AppointmentById_NotFound_Returns404()
    {
        var response = await _fixture.Client.GetAsync("/api/appointments/99999");
        response.StatusCode.Should().Be(HttpStatusCode.NotFound);
    }
    ...
    [Fact]
    public async Task PUT_CancelAppointment_Returns204()
    {
        var (p, t, r) = await SeedAsync("cancel");
        var createResponse = await BookAsync(p, t, r, Slot9am);
        var doc = await JsonDocument.ParseAsync(await createResponse.Content.ReadAsStreamAsync());
        var id = doc.RootElement.GetProperty("id").GetInt32();

        var updateResponse = await _fixture.Client.PutAsJsonAsync($"/api/appointments/{id}", new
        {
            StartTime = Slot9am,
            Status = "Canceled"
        });
    ...
    [Fact]
    public async Task PUT_CompleteAppointment_Returns204()
    {
        var (p, t, r) = await SeedAsync("complete");
        var createResponse = await BookAsync(p, t, r, Slot9am);
        var doc = await JsonDocument.ParseAsync(await createResponse.Content.ReadAsStreamAsync());
        var id = doc.RootElement.GetProperty("id").GetInt32();
